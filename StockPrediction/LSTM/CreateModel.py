...
model = ks.Sequential()
# model.add(ks.layers.LSTM(100, batch_input_shape=(batch_size, time_steps_in_batch, x_train.shape[2]), stateful=True,
# kernel_initializer='random_uniform'))test
model.add(ks.layers.LSTM(100, batch_input_shape=(batch_size, time_steps_in_batch, x_train.shape[2]), dropout=0.0,
...
history = model.fit(x_train, y_train, epochs=300, batch_size=batch_size, verbose=2, shuffle=False,
